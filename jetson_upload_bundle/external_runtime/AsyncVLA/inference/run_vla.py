...
class InferenceHandler:
    ...
    def goal_callback(self, msg):
        """Receive full text/image goals from the robot/goal Zenoh key.
        ...
        This lightweight run_vla.py still feeds the VLA with a language prompt.
        For image goals, the image is decoded and stored here, and the prompt is
        set to goal_text or a fallback emitted by the robot-side bridge. To make
        the model condition directly on goal_image, use the fuller AsyncVLA
        inference path that accepts goal_image_PIL.
        """
        try:
            payload = json.loads(msg.payload.to_bytes().decode("utf-8"))
            self.goal_mode = payload.get("mode", "text")
            self.lan_inst = payload.get("goal_text", "")

            goal_image = payload.get("goal_image")
            if goal_image and goal_image.get("data_b64"):
                img_bytes = base64.b64decode(goal_image["data_b64"])
                self.goal_image = Image.open(io.BytesIO(img_bytes)).convert("RGB")
                print("received image goal", self.goal_image.size)

            print("received goal", self.goal_mode, self.lan_inst)
            self.maybe_run()
        except Exception:
            traceback.print_exc()
...
def main():
    ...
    with zenoh.open(z_conf) as z_session:
        ...
        goal_subscriber = z_session.declare_subscriber("robot/goal", inference_handler.goal_callback)
